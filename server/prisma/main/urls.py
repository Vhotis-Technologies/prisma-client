...
from main.views.accept_invite import AcceptInviteView
...
from main.views.image_proxy import BookingImageProxyView, GuestBookingImageProxyView
...
urlpatterns = [
    ...
    # --- Guest checkout (public, rate-limited) ---
    path('guest/images/<uuid:image_id>/', GuestBookingImageProxyView.as_view(), name='guest_booking_image'),
    path('guest/<action>/', GuestBookingView.as_view(), name='guest_booking'),

    # --- Google Places proxy (public, rate-limited; key stays server-side) ---
    path('places/<action>/', PlacesView.as_view(), name='places'),
