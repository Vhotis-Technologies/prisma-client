...
from main.services.winner_voucher import amount_due_cents, compute_winner_discount

# Minimum purchasable gift voucher credit (EUR/GBP units, not cents).
MIN_GIFT_CREDIT_AMOUNT = Decimal("50")


def gift_purchase_amount_error(credit: Decimal) -> Optional[str]:
    """
    Return a user-facing error when ``credit`` is below the purchase minimum.

    Args:
        credit: Proposed voucher face value in major currency units.

    Returns:
        str | None: Error message, or None when the amount is valid.
    """
    if credit < MIN_GIFT_CREDIT_AMOUNT:
        return f"Minimum gift amount is {MIN_GIFT_CREDIT_AMOUNT:.0f}."
    return None
...
def validate_gift_voucher_for_payment(user, booking_data: dict, amount_cents: int) -> Optional[GiftVoucher]:
    ...
    pre_total = apply_server_pre_voucher_total(user, booking_data)
    discount = compute_gift_discount(voucher, pre_total)
    expected_cents = amount_due_cents(pre_total, discount)
    if abs(expected_cents - int(amount_cents)) > 2:
