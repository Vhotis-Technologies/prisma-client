...
def parse_pre_voucher_total(booking_data: dict) -> Decimal:
    """
    Read client ``pre_voucher_total_amount`` for apply-code preview only.

    Payment must use ``apply_server_pre_voucher_total`` instead of this value.

    Args:
        booking_data: Client booking dict.

    Returns:
        Decimal: Pre-discount total from the client.

    Raises:
        ValueError: When the field is missing.
    """
