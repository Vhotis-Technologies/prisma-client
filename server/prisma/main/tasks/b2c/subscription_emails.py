...
def send_b2c_subscription_payment_confirmation_email(
    ...
    """
    Invoice paid successfully — first cycle or renewal (invoice.payment_succeeded, non-duplicate path).
    """
    try:
...
def send_b2c_subscription_payment_due_reminder_email(
    ...
    """Stripe invoice.upcoming — next charge is coming soon. Optional Stripe hosted invoice page."""
    try:
...
def send_b2c_subscription_payment_failed_email(
    ...
    """invoice.payment_failed for B2C. ``update_payment_url`` retained for callers; links are omitted from email."""
    try:
